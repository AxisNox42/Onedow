...
// ─────────────────────────────────────────────────────────────
// C2_RELAY.sys — Command & Control (전면전)
//   · 코어(터미널+HOST) = C2 가짜 창 / adds = 네트워크 패킷(가짜창 없음)
//   · Minion 은 맵 전역 이동·교전, DrawAppWindow 없이 단순 도형만
//   · HOST 1기당 본체 피해 −11% (최대 88% 감소)
// ─────────────────────────────────────────────────────────────
class BotnetBoss {
public:
    float worldX, worldY;
    ...
    static constexpr float HOST_DMG_RED = 0.11f;   // HOST 1기당 본체 피해 −11%
